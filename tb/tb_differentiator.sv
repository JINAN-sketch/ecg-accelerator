...
module tb_differentiator;
    ...
    localparam PIPELINE  = 4;      // 4-sample shift register depth
    ...
    initial begin
        $readmemh("D:/Project_3/vectors/diff_input_hex.txt",  in_vec);
        $readmemh("D:/Project_3/vectors/diff_output_hex.txt", exp_vec);
        ...
        $display("==============================================");
        $display("  DIFFERENTIATOR TESTBENCH");
        $display("==============================================");
    ...
    always @(posedge clk) begin
        if (valid_out) begin
            if (out_idx >= PIPELINE) begin
                diff = $signed(y_out) - $signed(exp_vec[out_idx]);
                if (diff < 0) diff = -diff;
